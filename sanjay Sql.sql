#QUESTION 1
SELECT E_name FROM employee_Details WHERE E_name LIKE 'A%A';

#QUESTION 2
SELECT DISTINCT(E_name) FROM employee_details WHERE E_name IN (SELECT C_name FROM customer AS cus);

#QUESTION 3
CREATE VIEW PaymentNotDone AS SELECT * FROM logistics_Emp WHERE PAYMENT_STATUS = 'NOT PAID';
...
FROM Payment_Details GROUP BY PAYMENT_MODE;

#QUESTION 5
ALTER TABLE logistics_Emp ADD COLUMN TOTAL_PAYABLE_CHARGES FLOAT AFTER AMOUNT;
...
#QUESTION 9
SELECT SH_CONTENT, SUM(AMOUNT) AS Content_Wise_Amount FROM logistics_Emp GROUP BY (SH_CONTENT)
ORDER BY Content_Wise_Amount DESC LIMIT 5;

#QUESTION 10
SELECT SH_CONTENT, COUNT(SH_CONTENT) AS Content_Wise_Count FROM logistics_Emp GROUP BY(SH_CONTENT) 
ORDER BY Content_Wise_Count DESC LIMIT 5;

#QUESTION 11
CREATE VIEW TXLogistics AS SELECT * FROM logistics_Emp WHERE E_BRANCH = 'TX';
SELECT * FROM TXLogistics;

#QUESTION 12
ALTER VIEW TXLogistics AS SELECT *, AMOUNT - ((AMOUNT * 5)/100) AS New_Price FROM logistics_Emp
WHERE E_BRANCH = 'TX';
SELECT * FROM TXLogistics;
...
UPDATE logistics_Emp SET E_BRANCH = 'NJ' WHERE E_BRANCH = 'NY';
SELECT * FROM logistics_Emp;
...
#QUESTION 18
SELECT SERVICE_TYPE, COUNT(SERVICE_TYPE) AS Frequency FROM logistics_Emp GROUP BY SERVICE_TYPE
